...
#pdf(paste(gsea.gene.set,".pdf",sep=""),40,45)
  gsea.layout <- layout(matrix(c(1, 2, 3, 4)), heights = c(1.7, 0.5, 0.2, 2))
  layout.show(gsea.layout)
 
  # Create plots

  par(mar = c(0, 6, 2, 2),las=0)
  plot(c(1, gsea.hit.indices, length(gsea.rnk$metric)),
       c(0, gsea.es.profile, 0), type = "l",cex=1, col = "red", lwd = 2, xaxt = "n",
       xaxs = "i", xlab = "", ylab = "Enrichment score (ES)",
       ylim = enrichment.score.range,cex.lab=cex_value+0.2,cex.axis=cex_value,
       main = list(gsea.gene.set, font = 1, cex = cex_value+0.3),
       panel.first = {
  ...
  if(gsea.enrichment.score < 0) {  # ES值小于0的时候，画左下角
    text(length(gsea.rnk$metric) * 0.01, plot.coordinates[3] * 0.98,cex=cex_value+0.2,
...
#              gsea.normalized.enrichment.score, "\n"), adj = c(1, 1))
  ...
  plot(gsea.rnk$metric, type = "n", xaxs = "i",
	     ...
	     ylim = metric.range, yaxs = "i",cex.lab=cex_value+0.2,cex.axis=cex_value,
  ...
  barplot(rank.metric$values, col = "lightgrey", lwd = 0.1, xaxs = "i",
       xlab = "Rank in ordered gene list",cex.lab=cex_value+0.2,cex.axis=cex_value, xlim = c(0, length(gsea.rnk$metric)),
